...
默认宿主机端口：MySQL `13306`、Redis `16379`、Kafka `19092`、Elasticsearch `19200`、RustFS `19000`（控制台 `19001`）及 Mailpit `18025`。服务监听 `8080`，Compose 中可选的后端容器映射为 `18080`。
...
文件存储使用私有 RustFS Bucket，首次上传时自动创建配置的 Bucket。Compose 固定使用 RustFS 官方镜像的固定版本 `1.0.0`。上传接口仅接受 JPEG、PNG、WebP 和 GIF（最大 5 MiB），并只返回 15 分钟有效的预签名 URL；RustFS 暂时不可用时仅文件接口返回 `503`，不影响现有 readiness 契约。本地验证码邮件投递到 Mailpit；生产必须以 `CAMPUSHUB_MAIL_*`、`CAMPUSHUB_STORAGE_*` 和 `CAMPUSHUB_SECURITY_PII_*` 注入真实配置。
