...
> **实现状态（阶段 4）**：`tickets.totp_secret`、`tickets.totp_enabled`、`check_ins.check_in_no`、`check_ins.method`、`check_ins.snapshot` 目前尚未写入迁移。阶段 4 采用静态票据码 / 票据 UUID 核销，并用 `check_ins.client_request_id` 与 `check_ins.ticket_id` 的唯一键保证幂等；上述列留待后续实现 TOTP 与核销快照时再补。
...
> **实现状态（阶段 5）**：迁移已建 `chat_groups`、`chat_group_members`、`chat_messages` 三张表。`chat_groups.cover_file_id`/`cover_url`/`max_members`/`member_count`/`deleted_at` 与 `chat_messages.image_url` 暂未写入迁移：群成员数由 `chat_group_members` 实时统计，群封面沿用活动 `cover_url` 现签，`member_count` 与 `image_url` 留待需要时再补；`chat_messages.msg_type` 当前只使用 1（文本）和 2（图片）。`chat_group_events` 本阶段未建表，成员变化通过 `campushub.registration.events.v1` 消费与 `outbox_events` 审计。消息发送幂等由 `chat_messages` 的 `(sender_id, client_message_id)` 唯一键保证（阶段 5 的 WebSocket 发送沿用该约束）。
...
`uuid` 同时充当消费者幂等键：通知由 Kafka 事件派生，其 `uuid` 由 `event_id` 与收件人 ID 确定性生成，重复投递不会产生第二条通知。
